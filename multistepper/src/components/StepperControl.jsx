...
function StepperControl({handleClick,steps,currentStep}) {
  return (
    <div className='container flex justify-around mt-4 mb-8'>
      ...
      <button className='bg-green-500 text-white uppercase py-2 px-4 rounded-xl font-semibold cursor-pointer border-2 hover:bg-slate-700 hover:text-white transition duration-200 ease-in-out' onClick={()=>handleClick("next")}>{currentStep===steps.length-1?"Confirm":"Next"}</button>
    </div>
  )
}
